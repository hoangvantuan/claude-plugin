...
## Quick start
Set stable paths to the bundled CLI and the Python interpreter:

```
export IMAGE_GEN="skills/codex-image/scripts/image_gen.py"   # relative to the plugin repo root
alias python="$HOME/.venv/claude/bin/python"                  # venv with openai + pillow installed
```

Install dependencies into that venv: `uv pip install --python ~/.venv/claude/bin/python openai pillow`.
...
- Do not silently downgrade from CLI `gpt-image-2` or the default `codex exec` path to CLI `gpt-image-1.5`; ask first unless the user already explicitly requested `gpt-image-1.5`, `scripts/image_gen.py`, or CLI fallback.
...
When using this path, explain briefly that `codex exec` plus chroma-key removal is the default transparent-image path, but this request needs true model-native transparency. `gpt-image-2` does not support `background=transparent`, so `gpt-image-1.5` is required for this confirmed fallback.

## Quality, input fidelity, and masks (CLI fallback only)
These are explicit CLI controls. They are not available on the default `codex exec` path.
...
- Default-first transparent image workflow: `SKILL.md` and `scripts/remove_chroma_key.py`
