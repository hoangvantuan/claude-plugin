# Template TU — Thực Hành & Chuyển Hoá (70%)

**Mục đích:** Chuyển hoá kiến thức module này thành năng lực thực tế.

## Template `guided_practice.md`

```markdown
# Bài Thực Hành — [Tên Module]: [Tên Bài]

**Mục tiêu:** Sau bài này, bạn có thể [hành vi cụ thể]
**Thời gian:** [X] phút
**Kiến thức liên quan:** [Khái niệm từ phần Văn của module này]

---

## BƯỚC 1: QUAN SÁT (10 phút)

**Nhiệm vụ:** [Mô tả cụ thể điều cần quan sát]

| Quan sát | Ghi nhận |
|----------|----------|
| Tôi nhận thấy... | _______________ |
| Pattern tôi thấy... | _______________ |

---

## BƯỚC 2: PHÂN TÍCH (15 phút)

**Nhiệm vụ:** Áp dụng [framework module này] để phân tích.

[Bảng/ma trận/template phân tích cụ thể cho module này]

---

## BƯỚC 3: THỰC HÀNH (45 phút)

**Nhiệm vụ:** [Hướng dẫn step-by-step]

1. [ ] [Bước cụ thể 1]
2. [ ] [Bước cụ thể 2]
3. [ ] [Bước cụ thể 3]
4. [ ] Ghi lại kết quả

---

## BƯỚC 4: ĐÚC KẾT (10 phút)

| Câu hỏi | Trả lời |
|----------|---------|
| Điều quan trọng nhất tôi học được | ___ |
| Điều làm tốt, giữ lại | ___ |
| Điều làm khác lần sau | ___ |
| Kế hoạch áp dụng tiếp | ___ |
```

## Template `du_an_thuc_te.md`

```markdown
# Dự Án Thực Tế — [Tên Module]

## Brief
**Mục tiêu:** Áp dụng kiến thức module [N] vào 1 vấn đề thực tế.
**Thời gian:** [1-2 tuần]

**Yêu cầu đầu ra:**
1. [Deliverable cụ thể — liên quan đến chủ đề module này]
2. Bản đúc kết bài học (After-Action Review)

## Lộ trình
### Tuần 1: Chuẩn bị & Bắt đầu
- [ ] Chọn vấn đề thực tế để áp dụng kiến thức module này
- [ ] Viết mô tả vấn đề (Quan sát hiện trạng)
- [ ] Bắt đầu thực hành

### Tuần 2: Thực hành & Đúc kết
- [ ] Tiếp tục thực hành
- [ ] Ghi nhật ký phản tư (ít nhất 2 entry)
- [ ] Viết After-Action Review
- [ ] Chuẩn bị teach-back 5 phút

## Tiêu chí đánh giá

| Tiêu chí | Chưa đạt | Đạt | Xuất sắc |
|---|---|---|---|
| Áp dụng kiến thức module | Chưa dùng framework | Dùng đúng, kết quả hợp lý | Kết hợp sáng tạo |
| Đúc kết | Chung chung | Bài học cụ thể | Nguyên tắc tái sử dụng |
```

## Template `checklist_hanh_dong.md`

```markdown
# Checklist Hành Động — [Tên Module]

Dùng trong 1-2 tuần thực hành module này. Mỗi ngày 5-10 phút.

---

## Hành động mỗi ngày

- [ ] **Sáng:** [Hành động cụ thể liên quan đến module này]
- [ ] **Trong ngày:** [Hành động khi gặp tình huống liên quan]
- [ ] **Cuối ngày:** 3 phút review — Hôm nay tôi áp dụng được gì?

## Theo dõi tuần

| Thứ | Sáng | Trong ngày | Cuối ngày | Ghi chú |
|-----|------|------------|-----------|---------|
| T2  | ○    | ○          | ○         |         |
| T3  | ○    | ○          | ○         |         |
| T4  | ○    | ○          | ○         |         |
| T5  | ○    | ○          | ○         |         |
| T6  | ○    | ○          | ○         |         |

**Cuối tuần — Đúc kết:**
- Áp dụng tốt nhất: ___
- Cải thiện tuần tới: ___
```

## Template `teach_back.md` — Dạy Lại Để Hiểu Sâu

```markdown
# Teach-back — [Tên Module]

**Mục đích:** Dạy lại người khác là cách đúc kết mạnh nhất. Nếu bạn giải thích được → bạn hiểu thật.
**Thời gian trình bày:** 5 phút
**Đối tượng:** Buddy pair, nhóm nhỏ, hoặc facilitator

---

## Chuẩn bị (15-20 phút)

### 1. Chọn 1 bài học cốt lõi từ module
Điều quan trọng nhất tôi học được: ___

### 2. Cấu trúc trình bày (5 phút)

| Phần | Thời gian | Nội dung |
|------|-----------|----------|
| **Hook** | 30 giây | 1 câu hỏi hoặc tình huống gây tò mò |
| **Kiến thức** | 1 phút | Giải thích bài học bằng ngôn ngữ đơn giản nhất |
| **Ví dụ** | 2 phút | 1 ví dụ THỰC TẾ tôi đã trải nghiệm trong phần Tu |
| **Đúc kết** | 1 phút | 1 nguyên tắc người nghe có thể áp dụng ngay |
| **Hỏi đáp** | 30 giây | 1 câu hỏi mở cho người nghe |

### 3. Tự kiểm tra trước khi trình bày
- [ ] Tôi giải thích được MÀ KHÔNG CẦN đọc tài liệu?
- [ ] Ví dụ là từ trải nghiệm THẬT của tôi (không copy tài liệu)?
- [ ] Người không biết gì về chủ đề cũng hiểu được?

## Sau khi trình bày — Đúc kết

| Câu hỏi | Trả lời |
|----------|---------|
| Phần nào tôi giải thích trôi chảy nhất? | ___ |
| Phần nào tôi còn lúng túng? (= chưa hiểu sâu) | ___ |
| Câu hỏi nào từ người nghe khiến tôi phải suy nghĩ? | ___ |
| Tôi cần bổ sung kiến thức gì? | ___ |
```
