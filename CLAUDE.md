...
skills/                      # Các skill plugins (chi tiết xem ## Skill Structure)
  ...
  m365-meeting/              # Transcript họp Teams qua Graph: tìm đúng buổi (họp định kỳ trùng tên, họp mình chỉ tham dự), tải transcript ra .txt có header + .vtt gốc, recording, điểm danh, preflight chẩn đoán 403
  ...
  n8n-cli/                   # Quản lý n8n instance qua n8n-cli (REST API) — workflows, executions, credentials, projects, tags, variables, data tables, users, audit
...
| `m365-meeting`             | `references/` `scripts/` `tests/`                                    | Transcript Teams qua Graph. 2 đường: **bulk** `getAllTranscripts(meetingOrganizerUserId=...)` cho họp mình tổ chức, **per-thread** dựng `meetingId` = base64(`1*{oid}*0**{threadId}`) từ join URL cho họp mình tham dự (đã đo chạy thật). Scripts: `preflight.sh` (4 trạng thái chính sách tenant + scope trong token + câu nhắn IT), `list-transcripts.sh` (phân trang: trang 1 chỉ 6/9 item, `nextLink` sống sót cả khi trang rỗng), `meeting-resolver.py` (ghép transcript với lịch theo instant, chọn đúng buổi trong chuỗi lặp; subcommand `match`/`attended`/`meeting-id`/`decode`), `transcript-to-text.py` (VTT hoặc `metadataContent` → `.txt`, gộp cue cùng người <3s, cắt lượt >60s, giữ `@n`). Bẫy chính: `startDateTime`/`endDateTime` và `$filter` đều bị bỏ qua âm thầm. Tests offline trong `tests/` (51 test, `run-tests.sh`). Dừng ở tải file, không tóm tắt |
...
| `n8n-cli`                  | —                                                                    | Chỉ có SKILL.md. CLI quản lý n8n instance qua REST API: workflow (CRUD/activate/transfer), execution (list/retry/stop), credential (schema trước khi create), project/tag/variable/data-table/user, audit, source-control. Auto-JSON khi pipe, `--jq` filter, cấu hình qua login/config/env vars |
