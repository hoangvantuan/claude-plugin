...
skills/                      # Các skill plugins (chi tiết xem ## Skill Structure)
  ...
  project-memory/            # Bộ nhớ tri thức cấp dự án tự cải tiến — 4 op (capture/consolidate/recall/execute), 3 loại entry (Tool/Map/Fact), index routing + log timeline, script Python giữ index đồng bộ
...
| `project-memory`           | `references/` `scripts/`                                             | schemas, consolidation (lint + cross-ref 2 chiều), capture-signals, self-contained-exec. Scripts Python stdlib: _common, new-entry, reindex, archive, log. Dữ liệu ghi memory/ ở gốc repo. Tách 2 pha: tri thức (capture/consolidate) vs thực thi (execute) |
