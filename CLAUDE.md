...
skills/                      # Các skill plugins (chi tiết xem ## Skill Structure)
  ...
  planning-content/          # Phân tích nội dung & lên outline ý chính cho từng bài (audience, goal, angle, thesis, key points, data, takeaway). KHÔNG hướng dẫn cách viết/tone/style/structure
...
| `planning-content`         | —                                                                    | Chỉ có SKILL.md. Phân tích input → research → outline ý chính (1 bài = 1 chủ đề, có audience/goal/angle/thesis/key points/data/takeaway). Hỗ trợ batch cho input dài + content map + coverage check. KHÔNG can thiệp cách viết/tone/style/structure |
