...
`gpt-image-2` does not currently support the Image API `background=transparent` parameter. The skill's default transparent-image path is `codex exec` with a flat chroma-key background, followed by local alpha extraction with `~/.venv/claude/bin/python skills/codex-image/scripts/remove_chroma_key.py`.
