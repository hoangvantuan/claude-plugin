...
- `codex exec` with the `$imagegen` built-in tool (default)
- explicit `scripts/image_gen.py` CLI fallback (direct OpenAI API)
...
## Transparent images
- Use the default `codex exec` path first for transparent-image requests. If the subject is clearly too complex for chroma-key removal, explain the fallback and ask before switching to CLI.
...
- After generation, remove the background locally with `~/.venv/claude/bin/python skills/codex-image/scripts/remove_chroma_key.py --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill` and validate the alpha result before shipping it.
...
- Do not assume they are available on the default `codex exec` path — there, size/style/quality can only be hinted inside the prompt text.
...
## Style vocabulary quick tables

Compact vocabulary for filling the `Style/medium`, `Composition/framing`, and `Lighting/mood` schema lines.

Medium / style:

| Style          | Prompt wording                                      |
| -------------- | --------------------------------------------------- |
| Photorealistic | photorealistic, DSLR photograph, 85mm lens          |
| Flat design    | flat design illustration, vector style, clean lines |
...
Color palette examples: `pastel color palette, soft pink and light blue` · `monochrome, shades of gray` · `vibrant saturated colors, tropical palette` · `dark moody tones, deep navy and burgundy` · `earth tones, warm browns and greens`.

Anti-patterns:
- Too short (`a dog`) — the model decides everything.
- Too long (>500 words) — the model loses focus.
- Contradictory (`minimalist with lots of detail`) — pick one.
- Fake negative prompts (`no text, no watermark` alone) — describe what you DO want; state avoids as explicit constraints.
